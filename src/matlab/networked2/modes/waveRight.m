...
if(s < lift_period)
    u_theta_r = 0;
    u_phi_r = motorModel(0, alpha, lift_speed, s, lift_range);
else
    
    rotate_t = s - lift_period;
    u_theta_r = motorModel(0,alpha,rotate_speed,rotate_t,rotate_range);
    u_phi_r = 0;
    ...
    if(s > (lift_period + rotate_period))
        ...
        u_phi_r = motorModel(0,alpha,-lift_speed,lower_t,lift_range);
